...
export class World {
    ...
    command(name) {
        ...
        if (this.commands.length >= COMMAND_LIMIT)
            throw new Error(`Runaway program: more than ${COMMAND_LIMIT} commands recorded`);
        const s = this.sim;
        switch (name) {
            case "move-forward": {
    ...
    render() {
        ...
        this.camelia.innerHTML =
            `<span class="fx"><span class="upright"><span class="butterfly">` +
    ...
    upright(emoji, cls, gx, gy) {
        ...
        el.innerHTML = `<span class="upright">${emoji}</span>`;
        return el;
    }
    ...
    async stepOnce(stepMs = 300) {
        ...
        if (cmd.name === "turn-left" || cmd.name === "turn-right") this.arrowFacing = cmd.facing;
