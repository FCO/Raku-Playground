...
Guide Camelia 🦋 through *sagas* — sets of levels. The 16-level *Learn Raku* saga is a guided tour of Raku syntax (statements, `xx`, `for`, `until`, conditionals, subs, `given`/`when`, named parameters, `with` and more) with rich explanations on every level; five reference sagas cover *Quoting Constructs*, *Data Structures*, *Containers*, *Types & the MOP* and *Phasers*, following the pages at [docs.raku.org/reference](https://docs.raku.org/reference); *Regexes & Grammars* covers Raku's crown jewels — from `/gem/` to grammars with actions, with every match highlighted live in the preview pane; *Build Websites* teaches [MemoizedDOM](https://github.com/FCO/MemoizedDOM) — declarative, memoized web pages in Raku, rendered live in the preview pane; *Elevator Saga* is an event-driven port of [play.elevatorsaga.com](https://play.elevatorsaga.com/) — you write `init`/`update` handlers to run a whole building of elevators, simulated entirely in Raku; *Gem Rush* adds challenge islands. Locked progression per saga, adjustable playback speed, step-through, and a click-and-drag 3D world. New sagas are single JavaScript files dropped into `docs/sagas/`:
